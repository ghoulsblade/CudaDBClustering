...
__global__ static void dbscan_kernel_main (
	...
	//~ if (iPointID >= 0) {
		...
		float fSqDist,a;
		int d;
		for (d=0;d<D;++d) element[d] = pIn_Points[ARRAY_INDEX(max(0,iPointID) + d*N,N*D)]; // compiler should loop-unroll
		...
		for (n=0;n<iNumberOfPoints;++n) {
			...
			__syncthreads();
			if (threadIdx.x < D) pfSharedReadCache[ARRAY_INDEX(threadIdx.x,D)] = pIn_Points[ARRAY_INDEX(n + threadIdx.x*N,N*D)];
			__syncthreads();
...
void	dbscan_gpu	(DBScanData* p) {
	...
	#define MB(a) ((int)sizeof(a)/1024/1024)
	printf("alloc %d %d %d %d N=%d gridsize_x=%d\n",MB(p->pPoints),MB(p->pSeedList),MB(p->pClusterIDs),MB(p->pConnections),(int)N,grid_size.x);
